...
class ShowDifficulties < ClaudeHooks::UserPromptSubmit
  def call
    ...
    triggers = [ "show difficulties", "show your difficulties" ]
    ...
    if triggers.any? { |trigger| current_prompt.downcase.include?(trigger.downcase) }
      ...
      system_message!("\n--- \"Show Difficulties\" hook ran successfully ---")

      # Record a log line.
      log "Context Successfully Added: --> Show Difficulties"
    end
...
private
  ...
  def read_file
    ...
    file_path = project_path_for('hooks/handlers/user_prompt_submit/trigger_instructions/show-difficulties.txt')
...
if __FILE__ == $0
  ClaudeHooks::CLI.test_runner(ShowDifficulties)
end
