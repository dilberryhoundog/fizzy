...
private
  ...
  def format_line(line)
    template = case line
               ...
               in { type: 'user', toolUseResult: { agentId: String } }
  ...
  def rename_transcript(output_file)
    ...
    bash_cmd = %(claude -p "Read #{output_file}, analyze the conversation, edit the SUMMARY section with a descriptive summary of the conversation (max 5 lines, place between `>>>` `<<<`), create a concise descriptive title (max 6 words, kebab-case), then rename the file to <yy-mm-dd>_<title>.txt" --allowedTools "Read,Edit,Bash" --model haiku)
